...
if ( ! function_exists( 'woodmart_generate_posts_slider' ) ) {
	function woodmart_generate_posts_slider( $atts, $query = false, $products = false ) {
		...
		if ( $post_type == 'post' ) {
			$carousel_classes .= ' woodmart-carousel-spacing-' . $blog_spacing;
		}

		if ( $post_type == 'product' ) {
			$carousel_classes .= ' woodmart-carousel-spacing-' . $spacing;
		}

		if ( $post_type == 'portfolio' ) {
			$carousel_classes .= ' woodmart-carousel-spacing-' . $portfolio_spacing;
		}
		...
		if ( ( $query && $query->have_posts() ) || $products ) {
			...
			<div id="<?php echo esc_attr( $carousel_id ); ?>"
				 class="woodmart-carousel-container <?php echo esc_attr( $carousel_classes ); ?>" <?php echo ! empty( $owl_atts ) ? $owl_atts : ''; ?>>
				<?php
				if ( $element_title ) {
