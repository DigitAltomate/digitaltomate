...
class Image_Hotspot extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$repeater->add_control(
			...
			[
				'label'       => esc_html__( 'Dropdown side', 'xts-theme' ),
				'description' => esc_html__( 'Show the content on left or right side, top or bottom.', 'xts-theme' ),
				'type'        => Controls_Manager::SELECT,
				'options'     => [
		...
		$repeater->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} {{CURRENT_ITEM}}.woodmart-image-hotspot' => 'left: {{SIZE}}%;',
				],
			]
		);
		...
		$repeater->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} {{CURRENT_ITEM}}.woodmart-image-hotspot' => 'top: {{SIZE}}%;',
				],
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute(
			[
				'wrapper' => [
					'class' => [
						'woodmart-image-hotspot-wrapper',
		...
		if ( isset( $settings['image']['id'] ) && $settings['image']['id'] ) {
			...
			$image_output = '<img class="woodmart-image-hotspot-img" src="' . esc_url( $image_url ) . '">';
		} elseif ( isset( $settings['image']['url'] ) ) {
			$image_output = '<img class="woodmart-image-hotspot-img" src="' . esc_url( $settings['image']['url'] ) . '">';
		}
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<div class="woodmart-image-hotspot-hotspots">
				...
				<?php foreach ( $settings['items'] as $index => $item ) : ?>
					...
					?>
					<div class="woodmart-image-hotspot hotspot-type-<?php echo esc_attr( $settings['hotspot_type'] ); ?> elementor-repeater-item-<?php echo esc_attr( $item['_id'] ); ?>">
						...
						<?php if ( 'product' === $settings['hotspot_type'] && isset( $product ) && $product ) : ?>
							<div class="hotspot-product hotspot-content hotspot-dropdown-<?php echo esc_attr( $settings['hotspot_dropdown_side'] ); ?>">
								...
								<div class="hotspot-content-text woodmart-more-desc reset-mb-10">
									<div class="woodmart-more-desc-inner">
