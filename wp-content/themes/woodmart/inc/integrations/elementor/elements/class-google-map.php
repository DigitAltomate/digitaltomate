...
class Google_Map extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'      => [
					'{{WRAPPER}} .woodmart-google-map-content' => 'max-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		wp_enqueue_script( 'google.map.api', 'https://maps.google.com/maps/api/js?libraries=geometry&v=3.41&key=' . $settings['google_key'] . '', array(), '', false );
			...
			[
				'wrapper' => [
					'class' => [
						...
						'content-vertical-' . $settings['content_vertical'],
						'content-horizontal-' . $settings['content_horizontal'],
					],
				],
			]
		);
		...
		$map_args = array(
			...
			'init_offset'        => $settings['init_offset']['size'],
		...
		if ( isset( $settings['placeholder']['id'] ) && $settings['placeholder']['id'] ) {
			$placeholder = $image_output = woodmart_get_image_html( $settings, 'placeholder' );
		} else {
