...
class Products_Brands extends Widget_Base {
	...
	protected function render() {
		...
		if ( 'carousel' === $settings['style'] ) {
			...
			$this->add_render_attribute( 'wrapper', 'class', 'woodmart-carousel-spacing-0' );
			...
			$this->add_render_attribute( 'items_wrapper', 'class', 'woodmart-spacing-0' );
					...
					<?php foreach ( $brands as $key => $brand ) : ?>
						...
						<div <?php echo $this->get_render_attribute_string( 'items' ); ?>>
							<a title="Brand <?php echo esc_html( $brand->name ); ?>" href="<?php echo esc_url( $attr_link ); ?>">
								...
								<?php else : ?>
									<?php echo apply_filters( 'woodmart_image', '<img src="' . esc_url( $image ) . '" title="' . esc_attr( $brand->name ) . '" alt="' . esc_attr( $brand->name ) . '" />' ); ?>
								<?php endif; ?>
							</a>
						</div>
					<?php endforeach; ?>
