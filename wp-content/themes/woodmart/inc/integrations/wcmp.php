...
if ( ! function_exists( 'woodmart_wcmp_custom_css' ) ) {
	function woodmart_wcmp_custom_css() {
		if ( class_exists( 'WCMp' ) && is_vendor_dashboard() && ( is_user_wcmp_vendor( get_current_user_id() ) || is_user_wcmp_pending_vendor( get_current_user_id() ) || is_user_wcmp_rejected_vendor( get_current_user_id() ) ) ) {
			$css = apply_filters( 'woodmart_wcmp_custom_css', 'body[class*="wcmp"] .cart-widget-side,body[class*="wcmp"] .mobile-nav,body[class*="wcmp"] .woodmart-cookies-popup,body[class*="wcmp"] .pswp,body[class*="wcmp"] .scrollToTop,body[class*="wcmp"] .woodmart-search-full-screen,body[class*="wcmp"] .woodmart-promo-popup {display: none;} body[class*="wcmp"] .amount {color: inherit;}' );
