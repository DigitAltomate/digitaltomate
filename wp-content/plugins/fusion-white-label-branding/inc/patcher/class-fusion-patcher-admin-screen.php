				...
				<div class="fusion-patcher-heading">
					<p class="description">
						<?php if ( empty( $available_patches ) ) : ?>
							...
							<?php printf( esc_attr__( 'Fusion Patcher: Currently there are no patches available for %1$s version %2$s', 'fusion-white-label-branding' ), esc_attr( $this->patcher->get_args( 'name' ) ), esc_attr( $this->patcher->get_args( 'version' ) ) ); ?>
						<?php else : ?>
							<?php /* translators: The product name and its version. */ ?>
							<?php printf( esc_attr__( 'Fusion Patcher: The following patches are available for %1$s version %2$s', 'fusion-white-label-branding' ), esc_attr( $this->patcher->get_args( 'name' ) ), esc_attr( $this->patcher->get_args( 'version' ) ) ); ?>
						<?php endif; ?>
						<span class="fusion-auto-patcher learn-more"><a href="https://theme-fusion.com/avada-doc/avada-patcher/" target="_blank" rel="noopener noreferrer"><?php esc_attr_e( 'Learn More', 'fusion-white-label-branding' ); ?></a></span>
					</p>
					<?php if ( ! empty( $available_patches ) ) : ?>
						<p class="sub-description">
							<?php esc_attr_e( 'The status column displays if a patch was applied. However, a patch can be reapplied if necessary.', 'fusion-white-label-branding' ); ?>
						</p>
					<?php endif; ?>
				</div>
				<?php if ( ! empty( $messages ) ) : ?>
					<?php foreach ( $messages as $message_id => $message ) : ?>
						<?php if ( false !== strpos( $message_id, 'write-permissions-' ) ) : ?>
							<?php continue; ?>
						<?php endif; ?>
						<p class="fusion-patcher-error"><?php echo $message; ?></p>
					<?php endforeach; ?>
				<?php endif; ?>
					...
					<table class="fusion-patcher-table">
						<tbody>
							<tr class="fusion-patcher-headings">
								<th style="min-width:6em;"><?php esc_attr_e( 'Patch #', 'fusion-white-label-branding' ); ?></th>
								<th>
									<?php if ( ! empty( $bundles ) ) : ?>
										<?php esc_attr_e( 'Product', 'fusion-white-label-branding' ); ?>
									<?php else : ?>
										<?php esc_attr_e( 'Issue Date', 'fusion-white-label-branding' ); ?>
									<?php endif; ?>
								</th>
								<th><?php esc_attr_e( 'Description', 'fusion-white-label-branding' ); ?></th>
								<th><?php esc_attr_e( 'Status', 'fusion-white-label-branding' ); ?></th>
								<th></th>
							</tr>
							</tr>
										...
										<?php if ( $can_apply ) : ?>
											<form method="post" action="options.php">
												...
												<?php if ( $patch_applied ) : ?>
													<?php submit_button( esc_attr__( 'Patch Applied', 'fusion-white-label-branding' ) ); ?>
												<?php else : ?>
													<?php submit_button( esc_attr__( 'Apply Patch', 'fusion-white-label-branding' ) ); ?>
													<?php if ( $patch_failed ) : ?>
														...
														<a class="button" style="margin-top:10px;font-size:11px;color:#b71c1c;display:block;" href="<?php echo esc_url_raw( $dismiss_url ); ?>"><?php esc_attr_e( 'Dismiss Notices', 'fusion-white-label-branding' ); ?></a>
													<?php endif; ?>
												<?php endif; ?>
											</form>
										<?php else : ?>
											<span class="button disabled button-small">
												<?php if ( isset( $available_patches[ $key - 1 ] ) ) : ?>
													...
													<?php printf( esc_attr__( 'Please apply patch #%s first.', 'fusion-white-label-branding' ), intval( $available_patches[ $key - 1 ] ) ); ?>
												<?php else : ?>
													<?php esc_attr_e( 'Patch cannot be currently aplied.', 'fusion-white-label-branding' ); ?>
												<?php endif; ?>
											</span>
										<?php endif; ?>
									</td>
