 ...
 * @since 0.6
 */
final class WP_Service_Worker_Core_Asset_Caching_Component implements WP_Service_Worker_Component {
	...
	public function serve( WP_Service_Worker_Scripts $scripts ) {
		...
		$config = array(
			'route'      => '^' . preg_quote( trailingslashit( includes_url() ), '/' ) . '.*',
		...
		/**
		 * Filters service worker caching configuration for core asset requests.
		 *
		 * @since 0.6
		 *
		 * @param array {
		 *     Core asset caching configuration. If array filtered to be empty, then caching is disabled.
		 *
		 ...
		 *                                          The number 14 is derived from the 75th percentile of core assets used on pages served from
		 ...
		 * }
		 */
		$config = apply_filters( 'wp_service_worker_core_asset_caching', $config );
