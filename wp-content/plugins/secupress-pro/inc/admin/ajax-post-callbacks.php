...
function secupress_global_settings_white_label_ajax_post_cb() {
	...
	if ( ! defined( 'WP_SWL' ) || ! WP_SWL ) {
		wp_die(
			...
			'<p>' . sprintf( __( 'Sorry, you must define the %s constant first.', 'secupress-pro' ), '<code>WP_SWL</code>' ) . '</p>',
			403
		);
	}
	...
	if ( ! $old_email || ! $old_key || empty( $old_values['site_is_pro'] ) ) {
		secupress_add_settings_error( 'general', 'settings_updated', __( 'Sorry, you must activate your Pro license to use the White Label.', 'secupress-pro' ), 'error' );
...
add_action( 'admin_post_secupress_update_global_settings_settings-manager', 'secupress_global_settings_import_ajax_post_cb' );
/**
 ...
 * @author Julio Potier
 */
function secupress_global_settings_import_ajax_post_cb() {
	// Make all security tests.
	secupress_check_user_capability();
	secupress_check_admin_referer( 'secupress_update_global_settings_settings-manager' );
...
function secupress_geoips_update_data_ajax_post_cb() {
	...
	if ( is_null( secupress_get_module_option( 'geoip-system_type', null, 'firewall' ) ) ) {
		secupress_add_transient_notice( __( 'GeoIP database not updated.', 'secupress-pro' ) );
		wp_safe_redirect( wp_get_referer() );
		die();
	}
	if ( secupress_geoips_update_datas() ) {
		secupress_add_transient_notice( __( 'GeoIP database updated.', 'secupress-pro' ) );
	} else {
		secupress_add_transient_notice( __( 'GeoIP database not updated.', 'secupress-pro' ) );
	}
	wp_safe_redirect( wp_get_referer() );
