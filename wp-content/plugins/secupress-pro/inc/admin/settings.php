...
function secupress_pro_non_login_time_slot_field( $args, $instance ) {
	...
	<fieldset aria-labelledby="<?php echo $args['name']; ?>-time-slot-label">
		<legend class="screen-reader-text"><?php _e( 'Start hour and minute', 'secupress-pro' ); ?></legend>
		<label>
			<span class="label-before" aria-hidden="true"><?php _ex( 'From', 'starting hour + minute', 'secupress-pro' ); ?></span>
			...
			<span aria-hidden="true"><?php _ex( 'h', 'hour', 'secupress-pro' ); ?></span>
		</label>
		<label>
			...
			<span aria-hidden="true"><?php _ex( 'min', 'minute', 'secupress-pro' ); ?></span>
		</label>
	</fieldset>

	<fieldset aria-labelledby="<?php echo $args['name']; ?>-time-slot-label">
		<legend class="screen-reader-text"><?php _e( 'End hour and minute', 'secupress-pro' ); ?></legend>
		<label>
			<span class="label-before" aria-hidden="true"><?php _ex( 'To', 'ending hour + minute', 'secupress-pro' ) ?></span>
			<span class="screen-reader-text"><?php _e( 'Hour' ); ?></span>
...
function secupress_pro_file_scanner_field( $args, $instance ) {
	...
	if ( $running ) {
		$label = __( 'Stop task', 'secupress-pro' );
		$class = ' working';
		$turn  = 'off';
	} else {
		$label = __( 'Search for malicious files', 'secupress-pro' );
		$class = '';
		$turn  = 'on';
	}
	...
	?>
	<p>
		<a data-original-i18n="<?php esc_attr_e( 'Search for modified files', 'secupress-pro' ); ?>" data-loading-i18n="<?php esc_attr_e( 'Stop task', 'secupress-pro' ); ?>" id="toggle_file_scanner" href="<?php echo $url; ?>" class="secupress-button<?php echo $class; ?>">
			<?php echo $label; ?>
		</a>
	...
	if ( false === $files ) {
		?>
		<p class="description"><?php _e( 'This version of WordPress has not been scanned yet.', 'secupress-pro' ); ?></p>
		<?php
		return;
	} elseif ( ! array_filter( $files ) ) {
		?>
		<p class="description"><?php _e( 'Nothing found, well done.', 'secupress-pro' ); ?></p>
		<?php
		return;
	}
	...
	if ( $selfwhitelist ) {
		...
		$filelist_select_all = count( $selfwhitelist ) > 1;
		?>
		<form id="form-self-whitelist" action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_delete_selfwhitelist' ), 'secupress_delete_selfwhitelist' ) ); ?>" method="post">

			<h4><?php _e( 'The following files have been considered not harmful by yourself', 'secupress-pro' ); ?></h4>

			<fieldset id="secupress-group-selfwhitelist-files" class="secupress-boxed-group small-boxed-group secupress-check-group">

				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-selfwhitelist-file-1" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-selfwhitelist-file-1"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>

				<ul>
					<?php
					foreach ( $selfwhitelist as $file => $md5_array ) {
						printf(
							'<li><input id="selfwhitelist-file-%1$s" class="secupress-checkbox secupress-row-check" type="checkbox" name="files[]" value="%3$s"> <label for="selfwhitelist-file-%1$s"><strong>%2$s</strong></label> %4$s</li>',
							sanitize_html_class( $file ),
							esc_html( $file ),
							esc_attr( $file ),
							sprintf( __( 'Checksum MD5: %s', 'secupress-pro' ), key( $md5_array ) )
						);
					}
					?>
				</ul>
				...
				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-selfwhitelist-file-2" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-selfwhitelist-file-2"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>

			</fieldset>

			<p class="submit secupress-clearfix">
				<?php submit_button( __( 'Remove selected files from whitelist', 'secupress-pro' ), 'secondary alignright', 'submit-selfwhitelist-files', false ); ?>
			</p>
	...
	if ( ! empty( $files['not-wp-files'] ) ) {
		...
		?>
		<form id="form-delete-scanned-files" action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_action_on_scanned_files' ), 'secupress_action_on_scanned_files' ) ); ?>" method="post">

			<h4><?php _e( 'The followings are not files from WordPress core', 'secupress-pro' ); ?></h4>

			<p><span class="secupress-inline-alert"><?php _e( 'Possible malware found', 'secupress-pro' ); ?></span></p>
			...
			<fieldset id="secupress-group-diff-files" class="secupress-boxed-group small-boxed-group secupress-check-group">
				...
				<ul class="secupress-files-list">
					...
					foreach ( $files['not-wp-files'] as $diff_file ) {
						printf(
							'<li class="secupress-files-list-item"><input id="diff-file-%1$s" class="secupress-checkbox secupress-row-check" type="checkbox" name="files[]" value="%3$s"> <label for="diff-file-%1$s">%2$s</label>%4$s</li>',
							sanitize_html_class( $diff_file ),
							esc_html( $diff_file ),
							esc_attr( $diff_file ),
							secupress_malware_html() // All listed files have been spotted as possible malware: display the html directly.
						);
					} ?>
				</ul>
			...
			<p class="submit secupress-clearfix">
				...
				<?php submit_button( __( 'Add selected files in the whitelist', 'secupress-pro' ), 'secondary alignright', 'submit-whitelist-files', false ); ?>
			</p>

			<p>
				<span class="dashicons dashicons-warning" aria-hidden="true"></span> <em><?php _e( 'When you tag a file as "Not a malware", you will add this file in a whitelist in a smart way. Also it will send this information on our server.', 'secupress-pro' ); ?></em>
			</p>
	...
	if ( ! empty( $files['missing-wp-files'] ) ) {
		...
		<form id="form-recover-missing-files" action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_recover_missing_files' ), 'secupress_recover_missing_files' ) ); ?>" method="post">
			...
			<h4><?php _e( 'The followings are missing from WordPress core files', 'secupress-pro' ); ?></h4>

			<fieldset id="secupress-group-miss-files" class="secupress-boxed-group small-boxed-group secupress-check-group">

				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-miss-file-1" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-miss-file-1"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>
				...
				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-miss-file-2" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-miss-file-2"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>

			</fieldset>

			<p class="submit secupress-clearfix">
				<?php submit_button( __( 'Recover selected files', 'secupress-pro' ), 'secondary alignright secupress-button secupress-button-mini', 'submit-recover-missing-files', false ); ?>
			</p>
	...
	if ( ! empty( $files['old-wp-files'] ) ) {
		...
		<form id="form-old-files" action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_old_files' ), 'secupress_old_files' ) ); ?>" method="post">
			...
			<h4><?php _e( 'The followings are old WordPress core files', 'secupress-pro' ); ?></h4>
			...
			<fieldset id="secupress-group-old-files" class="secupress-boxed-group small-boxed-group secupress-check-group">
				...
				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-old-file-1" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-old-file-1"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>
				...
				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-old-file-2" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-old-file-2"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>

			</fieldset>

			<p class="submit secupress-clearfix">
				<?php submit_button( __( 'Delete selected files', 'secupress-pro' ), 'secondary alignright secupress-button secupress-button-mini', 'submit-recover-diff-files', false ); ?>
			</p>
	...
	if ( ! empty( $files['modified-wp-files'] ) ) {
		...
		foreach ( $files['modified-wp-files'] as $mod_file ) {
			...
			$filelist_li .= sprintf(
				'<li class="secupress-files-list-item"><input id="mod-file-%1$s" class="secupress-checkbox secupress-row-check" type="checkbox" name="files[]" value="%3$s" title="%3$s"> <label for="mod-file-%1$s" title="%3$s">%2$s</label> <a target="_blank" href="%4$s" class="secupress-button secupress-button-ghost secupress-button-mini secupress-button-primary"><span class="icon"><i class="dashicons dashicons-plus" aria-hidden="true"></i></span><span class="text">' . __( 'See differences', 'secupress-pro' ) . '</span></a>%5$s</li>',
		...
		<form id="form-recover-diff-files" action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_action_on_scanned_files' ), 'secupress_action_on_scanned_files' ) ); ?>" method="post">
			...
			<h4><?php _e( 'The followings are modified WordPress core files', 'secupress-pro' ); ?></h4>
			...
			<fieldset id="secupress-group-mod-files" class="secupress-boxed-group small-boxed-group secupress-check-group">
				...
				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-mod-file-1" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-mod-file-1"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>
				...
				<?php if ( $filelist_select_all ) { ?>
					<span class="hide-if-no-js"><input id="secupress-toggle-check-mod-file-2" type="checkbox" class="secupress-checkbox secupress-toggle-check"> <label for="secupress-toggle-check-mod-file-2"><em><?php _e( '(Un)Select All', 'secupress-pro' ); ?></em></label></span>
				<?php } ?>

			</fieldset>

			<p class="submit secupress-clearfix">
				<?php submit_button( __( 'Recover selected files', 'secupress-pro' ), 'secondary alignright', 'submit-recover-diff-files', false ); ?>
			</p>
...
function secupress_pro_backup_history_field( $args, $instance ) {
	...
	?>
	<p id="secupress-no-backups"<?php echo $backup_files ? ' class="hidden"' : ''; ?>><em><?php _e( 'No Backups found yet, do one?', 'secupress-pro' ); ?></em></p>
	...
	<form id="form-delete-backups"<?php echo ! $backup_files ? ' class="hidden"' : ''; ?> action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_delete_backups' ), 'secupress_delete_backups' ) ); ?>" method="post">
		...
		<strong id="secupress-available-backups"><?php printf( _n( '%s available Backup', '%s available Backups', count( $backup_files ), 'secupress-pro' ), number_format_i18n( count( $backup_files ) ) ); ?></strong>

		<fieldset class="secupress-boxed-group">
			<legend class="screen-reader-text"><span><?php _e( 'Backups', 'secupress-pro' ); ?></span></legend>
		...
		<p class="submit">
			<button class="secupress-button secupress-button-secondary alignright" type="submit" id="submit-delete-backups">
				...
				<span class="text">
					<?php _e( 'Delete all Backups', 'secupress-pro' ); ?>
				</span>
			</button>
		</p>
...
function secupress_pro_backup_db_field( $args, $instance ) {
	...
	<form action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_backup_db' ), 'secupress_backup_db' ) ); ?>" id="form-do-db-backup" method="post">
		...
		<fieldset class="secupress-boxed-group">
			<legend class="screen-reader-text"><span><?php _e( 'DataBase Tables', 'secupress-pro' ); ?></span></legend>

			<b><?php _e( 'Unknown tables', 'secupress-pro' ); ?></b>
			<br>
			<?php
			if ( $other_tables ) {
				...
				_ex( 'None', 'database table', 'secupress-pro' );
			}
			?>
			<hr>
			<b><?php _e( 'WordPress tables (mandatory)', 'secupress-pro' ); ?></b>
			<br>
			<?php
			if ( $wp_tables ) {
		...
		<p class="submit">
			<button class="secupress-button" type="submit" data-original-i18n="<?php esc_attr_e( 'Backup my Database', 'secupress-pro' ); ?>" data-loading-i18n="<?php esc_attr_e( 'Backuping&hellip;', 'secupress-pro' ); ?>" id="submit-backup-db">
				<span class="icon">
					<i class="secupress-icon-download" aria-hidden="true"></i>
				</span>
				<span class="text">
					<?php _e( 'Backup my Database', 'secupress-pro' ); ?>
				</span>
			</button>
...
function secupress_pro_backup_files_field( $args, $instance ) {
	...
	<form action="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=secupress_backup_files' ), 'secupress_backup_files' ) ); ?>" id="form-do-files-backup" method="post">
		...
		<fieldset>
			<legend><strong><label for="ignored_directories"><?php _e( 'Do not backup the following files and folders:', 'secupress-pro' ); ?></label></strong></legend>
			<br>
			...
			<p class="description">
				<?php _e( 'One file or folder per line.', 'secupress-pro' ); ?>
			</p>
		</fieldset>

		<p class="submit">
			<button class="secupress-button" type="submit" data-original-i18n="<?php esc_attr_e( 'Backup my Files', 'secupress-pro' ); ?>" data-loading-i18n="<?php esc_attr_e( 'Backuping&hellip;', 'secupress-pro' ); ?>" id="submit-backup-files">
				<span class="icon">
					<i class="secupress-icon-download" aria-hidden="true"></i>
				</span>
				<span class="text">
					<?php _e( 'Backup my Files', 'secupress-pro' ); ?>
				</span>
			</button>
			<span class="spinner secupress-inline-spinner"></span>
		</p>
...
function secupress_pro_scheduled_backups_field( $args, $instance ) {
	...
	if ( ! $next_schedule ) {
		_e( 'Nothing\'s scheduled yet.', 'secupress-pro' );
		return;
	}

	// Date.
	$next_schedule = date_i18n( _x( 'l, F jS, Y \a\t h:ia', 'Schedule date', 'secupress-pro' ), $next_schedule );
	...
	if ( isset( $types['db'] ) ) {
		$type[] = __( 'the database', 'secupress-pro' );
	}
	if ( isset( $types['files'] ) ) {
		$type[] = __( 'the files', 'secupress-pro' );
	}
	...
	printf(
		...
		__( 'Next backup will occur %1$s and will backup %2$s.', 'secupress-pro' ),
	...
	if ( $email ) {
		printf(
			...
			__( 'This task will %1$s, and once finished you will be notified at: %2$s.', 'secupress-pro' ),
			'<span class="secupress-highlight">' . sprintf( _n( 'repeat every %d day', 'repeat every %d days', $periodicity, 'secupress-pro' ), $periodicity ) . '</span>',
			'<span class="secupress-highlight">' . $email . '</span>'
		);
	} else {
		printf(
			/** Translators: %s is "repeat every X days". */
			__( 'This task will %s.', 'secupress-pro' ),
			'<span class="secupress-highlight">' . sprintf( _n( 'repeat every %d day', 'repeat every %d days', $periodicity, 'secupress-pro' ), $periodicity ) . '</span>'
		);
	}
}
...
function secupress_pro_scheduled_scan_field( $args, $instance ) {
	...
	printf(
		...
		__( 'Next scan will occur %s.', 'secupress-pro' ),
		'<span class="secupress-highlight">' . $next_schedule . '</span>'
	);
