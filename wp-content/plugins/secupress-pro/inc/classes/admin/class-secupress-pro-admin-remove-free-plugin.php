...
class SecuPress_Pro_Admin_Remove_Free_Plugin {
	...
	protected function make_free_plugin_dummy() {
		...
		$dummy .= ' * Plugin Name: ' . __( 'SecuPress — WordPress Security', 'secupress-pro' ) . "\n";
		$dummy .= ' * Description: ' . __( 'PLEASE DELETE ME <strong>VIA FTP</strong>.', 'secupress-pro' ) . "\n";
		$dummy .= " * Version: 100.0\n";
		$dummy .= " * Network: true\n";
		$dummy .= " */\n";
	...
	public function prevent_free_plugin_action() {
		...
		if ( ! empty( $_GET['plugins'] ) ) {
			...
			if ( false !== $key ) {
				...
				wp_die( sprintf( __( 'Performing actions with %s is forbidden, unless you want the sky to fall to your head.', 'secupress-pro' ), '<strong>SecuPress</strong>' ) );
			}
		}
	...
	public function prevent_ajax_plugin_update() {
		...
		wp_send_json_error( array(
			...
			'errorMessage' => sprintf( __( 'There is no need to update this plugin anymore, %s is now working without the free plugin (cool uh?).', 'secupress-pro' ), SECUPRESS_PLUGIN_NAME ),
		) );
	}
	...
	public function display_free_plugin_notice() {
		...
		echo '<div class="error"><p>' . sprintf( __( '%1$s couldn\'t delete the free plugin. Please delete the folder %2$s via FTP.', 'secupress-pro' ), '<strong>SecuPress Pro</strong>', "<code>{$path}</code>" ) . '</p></div>' . "\n";
	}
	...
	public function remove_free_plugin_action_links( $actions ) {
		...
		if ( isset( $actions['delete'] ) ) {
			$new_actions['delete'] = '<del class="delete">' . __( 'Delete' ) . '</del>'; // WP i18n.
		}
	...
	public function add_warning_after_free_plugin_row() {
		...
		echo '<div class="error-message notice inline notice-alt notice-error"><p>' . sprintf( __( 'Please delete the folder %s via FTP.', 'secupress-pro' ), "<code>{$path}</code>" ) . '</p></div>';
		echo "</td></tr>\n";
	}
