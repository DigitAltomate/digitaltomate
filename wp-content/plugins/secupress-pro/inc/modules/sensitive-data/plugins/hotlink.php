...
function secupress_hotlink_activation() {
	...
	if ( ! secupress_is_site_ssl() ) {
		$message  = sprintf( __( '%s:', 'secupress-pro' ), __( 'Anti Hotlink', 'secupress-pro' ) ) . ' ';
		$message .= __( 'The anti hotlink can work only over SSL (the URL of your website must start with <code>https://</code>).', 'secupress-pro' );
	...
	secupress_add_module_rules_or_notice( array(
		'rules'  => $rules,
		'marker' => 'hotlink',
		'title'  => __( 'Anti Hotlink', 'secupress-pro' ),
	) );
}
...
function secupress_hotlink_deactivate() {
	secupress_remove_module_rules_or_notice( 'hotlink', __( 'Anti Hotlink', 'secupress-pro' ) );
}
