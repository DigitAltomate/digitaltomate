...
function secupress_retrieve_password( $user_login ) {
	...
	if ( $message && ! wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
		wp_die( __( 'The email could not be sent.', 'secupress-pro' ) . "<br />\n" . __( 'Possible reason: your host may have disabled the mail() function.', 'secupress-pro' ) );
	}
...
function secupress_admin_post_resetpassword() {
	...
	if ( ! current_user_can( 'edit_user', $user_id ) ) {
		secupress_add_transient_notice( __( 'Password has not been reset and email has not been sent.', 'secupress-pro' ), 'error', 'secupress-resetpassword' );
	...
	secupress_add_transient_notice( __( 'Password has been reset and email has been sent.', 'secupress-pro' ), 'updated', 'secupress-resetpassword' );
	wp_redirect( wp_get_referer() );
	die();
}
...
function secupress_pro_sessions_control_init() {
	...
	add_action( 'all_admin_notices',          'secupress_pro_sessions_control_destroy_user_sessions_cb' );
	add_action( 'all_admin_notices',          'secupress_pro_sessions_control_destroy_all_sessions_cb' );
...
function secupress_pro_sessions_control_print_styles() {
	$width = _x( '13', 'sessions column width in "em"', 'secupress-pro' );
...
function secupress_pro_sessions_control_enqueue_scripts() {
	...
	wp_localize_script( 'secupress-sessions-control', 'spSessionsControlL10n', array(
		...
		'currentUserCellText'   => __( 'You are only logged in at this location.' ), // WP i18n.
		'otherUsersCellText'    => __( 'Disconnected', 'secupress-pro' ),
		'destroySessionsText'   => __( 'Destroy sessions', 'secupress-pro' ),
...
function secupress_pro_sessions_control_display_destroy_all_button() {
	$url     = 'users.php' . ( ! empty( $_GET['paged'] ) ? '?paged=' . (int) $_GET['paged'] : '' );
	$url     = self_admin_url( add_query_arg( array( 'action' => 'secupress-destroy-all-sessions' ), $url ) );
	$url     = wp_nonce_url( $url, 'secupress-destroy-all-sessions-' . get_current_user_id() );

	echo '<a href="' . esc_url( $url ) . '" class="button secupress-destroy-all-sessions">' . __( 'Destroy all sessions', 'secupress-pro' ) . '</a>';
}
...
function secupress_pro_sessions_control_users_column_title( $columns ) {
	$columns['secupress-sessions'] = __( 'Sessions' ); // WP i18n.
	return $columns;
}
...
function secupress_pro_sessions_control_users_column_content( $output, $column_name, $user_id ) {
	...
	// No sessions.
	if ( ! $all_sessions ) {
		return '<em>' . __( 'Disconnected', 'secupress-pro' ) . '</em>';
	}

	$count_sessions = count( $all_sessions );

	$url = 'users.php' . ( ! empty( $_GET['paged'] ) ? '?paged=' . (int) $_GET['paged'] : '' );
	$url = self_admin_url( add_query_arg( array( 'action' => 'secupress-destroy-user-sessions', 'users[]' => $user_id ), $url ) );
	$url = wp_nonce_url( $url, 'secupress-destroy-user-sessions-' . get_current_user_id() );

	// The current user.
	if ( get_current_user_id() === $user_id ) {
		if ( 1 === $count_sessions ) {
			return '<em>' . __( 'You are only logged in at this location.' ) . '</em>'; // WP i18n.
		}

		$output  = '<a href="' . esc_url( $url ) . '" class="button button-secondary secupress-destroy-sessions">' . __( 'Destroy other sessions', 'secupress-pro' ) . '</a>';
		$output .= '<p class="description">' . sprintf( __( 'You have %s sessions opened.', 'secupress-pro' ), $count_sessions ) . "</p>\n";
	}
	// Other users.
	else {
		$output  = '<a href="' . esc_url( $url ) . '" class="button button-secondary secupress-destroy-sessions">' . _n( 'Destroy session', 'Destroy all sessions', $count_sessions, 'secupress-pro' ) . '</a>';
		$output .= '<p class="description">' . sprintf( _n( '%s session opened', '%s sessions opened', $count_sessions, 'secupress-pro' ), $count_sessions ) . "</p>\n";
	}
...
function secupress_pro_sessions_control_destroy_user_sessions_cb() {
	if ( empty( $_GET['action'] ) || 'secupress-destroy-user-sessions' !== $_GET['action'] ) {
		return;
	}
	...
	if ( empty( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'secupress-destroy-user-sessions-' . get_current_user_id() ) ) {
		wp_die( __( 'Cheatin&#8217; uh?' ) ); // WP i18n.
		return;
	}
	...
	// Send a response.
	$args = array( 'update' => 'destroyed_sessions' );
...
function secupress_pro_sessions_control_destroy_all_sessions_cb() {
	if ( empty( $_GET['action'] ) || 'secupress-destroy-all-sessions' !== $_GET['action'] ) {
		return;
	}

	if ( empty( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'secupress-destroy-all-sessions-' . get_current_user_id() ) ) {
		wp_die( __( 'Cheatin&#8217; uh?' ) ); // WP i18n.
		return;
	}
...
function secupress_pro_sessions_control_display_notice() {
	...
	switch ( $_GET['update'] ) :
		case 'err_destroy_sessions_user' :
			...
			echo '<div id="secupress-sessions-control-notice" class="error notice is-dismissible"><p>' . __( 'Error: wrong user.', 'secupress-pro' ) . '</p></div>';
			break;

		case 'destroyed_sessions' :
			// Several users.
			if ( empty( $_GET['id'] ) ) {
				echo '<div id="secupress-sessions-control-notice" class="updated notice is-dismissible"><p>' . __( 'Selected users have been logged out.', 'secupress-pro' ) . '</p></div>';
				break;
			}
		...
		case 'destroyed_all_sessions' :
			// All users.
			echo '<div id="secupress-sessions-control-notice" class="updated notice is-dismissible"><p>' . __( 'You are now logged out everywhere else and all other users have been logged out.', 'secupress-pro' ) . '</p></div>';
			break;
	endswitch;
}
...
function secupress_pro_sessions_control_destroy_user_sessions_ajax_cb() {
	...
	if ( ! current_user_can( secupress_pro_sessions_control_get_capability() ) || ! current_user_can( 'list_users' ) ) {
		wp_send_json_error( array( 'message' => __( 'You do not have sufficient permissions to access this page.' ) ) ); // WP i18n.
	}

	if ( false === check_ajax_referer( 'secupress-destroy-user-sessions-' . get_current_user_id() ) ) {
		wp_send_json_error( array( 'message' => __( 'Cheatin&#8217; uh?' ) ) ); // WP i18n.
	}
	...
	foreach ( $users as $i => $user ) {
		...
		if ( ! secupress_is_user( $user ) ) {
			if ( $single_user ) {
				// Asked for only one user, let's display an error message.
				wp_send_json_error( array( 'message' => __( 'Error: wrong user.', 'secupress-pro' ) ) );
			}
	...
	// Send a response.
	if ( ! $single_user ) {
		$message = __( 'Selected users have been logged out.', 'secupress-pro' );
...
function secupress_pro_sessions_control_destroy_all_sessions_ajax_cb() {
	...
	// Send a response.
	wp_send_json_success( array( 'message' => __( 'You are now logged out everywhere else and all other users have been logged out.', 'secupress-pro' ) ) );
}
