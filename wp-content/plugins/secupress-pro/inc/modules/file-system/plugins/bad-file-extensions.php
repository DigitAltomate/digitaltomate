...
add_action( 'secupress.modules.activate_submodule_' . basename( __FILE__, '.php' ), 'secupress_bad_file_extensions_activation' );
/**
...
function secupress_bad_file_extensions_activation() {
	...
	secupress_add_module_rules_or_notice( array(
		...
		'title'  => __( 'Bad File Extensions', 'secupress-pro' ),
	) );
}
...
function secupress_bad_file_extensions_deactivate() {
	secupress_remove_module_rules_or_notice( 'bad_file_extensions', __( 'Bad File Extensions', 'secupress-pro' ) );
}
...
function secupress_bad_file_extensions_get_regex_pattern() {
	...
	$media_url = str_replace( $abspath, '', $media_url ); // 'wp-content/uploads/'
	$media_url = $bases['home_from'] . $media_url;

	$extensions = secupress_bad_file_extensions_get_forbidden_extensions();
	$extensions = implode( '#//#', $extensions );
	$extensions = addcslashes( $extensions, '[](){}.*+?|^$' );
	$extensions = str_replace( '#//#', '|', $extensions );

	return "^{$media_url}.*\.($extensions)$";
}
