...
function secupress_backup_files_ajax_post_cb() {
	...
	if ( ! class_exists( 'ZipArchive' ) ) {
		secupress_admin_die( __( 'Class <code>ZipArchive</code> is missing, le zip archive cannot be created.', 'secupress-pro' ) );
	}

	$backup_file = secupress_do_files_backup();

	if ( ! $backup_file ) {
		secupress_admin_die( __( 'No backup file created.', 'secupress-pro' ) );
	}
	...
	secupress_admin_send_response_or_redirect( array(
		...
		'countText'    => sprintf( _n( '%s available Backup', '%s available Backups', count( $backup_files ), 'secupress-pro' ), number_format_i18n( count( $backup_files ) ) ),
	) );
}
...
function secupress_print_backup_file_formated( $file, $echo = true ) {
	...
	$_date   = date_i18n( __( 'M jS Y', 'secupress-pro' ) . ' ' . __( 'G:i', 'secupress-pro' ), $_date, true );
	$_prefix = str_replace( array( '@', '#' ), array( '.', '/' ), $_prefix );

	switch ( $_type ) :
		case 'database' :
			$_type = __( 'Database', 'secupress-pro' );
		break;

		case 'files' :
			$_type = __( 'Files', 'secupress-pro' );
		break;
	...
	$file_format .= sprintf( '<span><a href="%s">%s</a> | <a href="%s" class="a-delete-backup" data-file-uniqid="%s">%s</a></span></p>', $download_url, _x( 'Download', 'verb', 'secupress-pro' ), $delete_url, $file_uniqid, __( 'Delete', 'secupress-pro' ) );
