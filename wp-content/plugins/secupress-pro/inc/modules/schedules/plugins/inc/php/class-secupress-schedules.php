...
class SecuPress_Schedules extends SecuPress_Singleton {
	...
	public function add_recurrence( $new_schedules ) {
		...
		if ( 'daily' !== $recurrence && empty( $new_schedules[ $recurrence ] ) ) {
			...
			$new_schedules[ $recurrence ] = array( 'interval' => $days * DAY_IN_SECONDS, 'display' => sprintf( _n( 'Every %s Day', 'Every %s Days', $days, 'secupress-pro' ), number_format_i18n( $days ) ) );
		}
	...
	protected function get_email_strings( $args = array() ) {
		if ( $args['success'] ) {
			return array(
				...
				'subject' => sprintf( __( '[%s] Operation succeeded', 'secupress-pro' ), '###SITENAME###' ),
				'message' => __( 'The scheduled operation of your site has succeeded!', 'secupress-pro' ),
			);
		}

		return array(
			/** Translators: %s is the blog name. */
			'subject' => sprintf( __( '[%s] Operation failed', 'secupress-pro' ), '###SITENAME###' ),
			'message' => __( 'The scheduled operation of your site has failed.', 'secupress-pro' ),
		);
	}
}
