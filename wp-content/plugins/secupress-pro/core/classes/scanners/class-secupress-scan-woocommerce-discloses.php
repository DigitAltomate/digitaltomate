...
class SecuPress_Scan_Woocommerce_Discloses extends SecuPress_Scan implements SecuPress_Scan_Interface {
	...
	public static function get_messages( $message_id = null ) {
		...
		$messages = array(
			...
			// "warning"
			/** Translators: %s is a plugin name. */
			100 => sprintf( __( 'Unable to determine if %s is disclosing its version on your homepage.', 'secupress' ), '<strong>WooCommerce</strong>' ) . ' ' . $activate_protection_message,
			// "bad"
	...
	public function fix() {
		// Activate.
		secupress_activate_submodule( 'discloses', 'woocommerce-version' );
