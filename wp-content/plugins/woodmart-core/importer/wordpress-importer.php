...
define( 'WOODCORE_IMPORT_DEBUG', false );
...
<?php
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ( defined('WOODCORE_IMPORT_DEBUG') && WOODCORE_IMPORT_DEBUG )
	...
	function fetch_remote_file( $url, $post ) {
		...
		$upload_date = isset( $post['upload_date'] ) ? $post['upload_date'] : '';
		...
		$this->url_remap[$guid] = $upload['url']; // r13735, really needed?
