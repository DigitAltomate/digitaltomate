 ...
 */
class AmeliaGoogle_Logger_File extends AmeliaGoogle_Logger_Abstract
{
  /**
  ...
  public function __construct(AmeliaGoogle_Client $client)
  {
    ...
    $file = $client->getClassConfig('AmeliaGoogle_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('AmeliaGoogle_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('AmeliaGoogle_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new AmeliaGoogle_Logger_Exception('File pointer is no longer available');
    }
