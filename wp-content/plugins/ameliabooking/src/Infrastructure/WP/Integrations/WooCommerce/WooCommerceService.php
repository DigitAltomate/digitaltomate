...
class WooCommerceService
{
            ...
            if ($result->getResult() === CommandResult::RESULT_SUCCESS) {
                ...
                BookingAddedEventHandler::handle(
                    $reservationService->getSuccessBookingResponse(
                        ...
                        !empty($data['packageId']) ? $data['packageId'] : null,
            ...
            case (Entities::APPOINTMENT):
                ...
                foreach ($wcItemAmeliaCache['recurring'] as $index => $recurringReservation) {
                    ...
                    if ($index < $bookableData['bookable']['recurringPayment']) {
                        $paymentAmount += self::getBookingPaymentAmount(
                            array_merge(
                                $wcItemAmeliaCache,
                                [
                                ...
                                Entities::APPOINTMENT => array_merge(
                                    ...
                                    [
                                        ...
                                        'bookings' => [
                                            array_merge(
                                                $wc_item[self::AMELIA]['bookings'][0],
                                                ['utcOffset' => $packageReservation['utcOffset'], 'price' => 0]
                                            )
                                        ],
                                    ]
                                ),
    ...
    public static function getLabels($data)
    {
        if ($data && is_array($data)) {
            ...
            return '<br/>' . $other_data[0]['name'] . '<br/>' . nl2br($other_data[0]['value']);
        }
    }
