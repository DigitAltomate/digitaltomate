...
abstract class PlaceholderService implements PlaceholderServiceInterface
{
    ...
    public function getCustomFieldsData($appointment, $bookingKey = null)
    {
        ...
        if ($bookingKey === null) {
            $sendAllCustomFields = $settingsService->getSetting('notifications', 'sendAllCF');
            foreach ($appointment['bookings'] as $booking) {
                if ((!$booking['isChangedStatus'] || (array_key_exists('isLastBooking', $booking) ? !$booking['isLastBooking'] : false))
