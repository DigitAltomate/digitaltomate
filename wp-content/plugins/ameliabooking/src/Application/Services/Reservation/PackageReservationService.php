...
class PackageReservationService extends AppointmentReservationService
{
    ...
    public function book($appointmentData, $inspectTimeSlot, $save)
    {
        ...
        $clonedCustomFieldsData = json_decode($appointmentData['bookings'][0]['customFields'], true);
        ...
        foreach ($appointmentData['package'] as $index => $packageData) {
            ...
            try {
                ...
                $this->bookSingle($packageReservation, $packageAppointmentData, $inspectTimeSlot, $save);
            } catch (Exception $e) {
                if ($save) {
                    /** @var Reservation $packageReservation */
        ...
        if ($save) {
            $this->addPayment(
                ...
                $this->getReservationPaymentAmount($reservation),
        ...
        return [
            ...
            'bookings'           => [
                [
                    ...
                    'persons'      => 1,
                    'extras'       => [],
                    'status'       => $booking ? $booking->getStatus()->getValue() : null,
                    'utcOffset'    => $booking && $booking->getUtcOffset() ? $booking->getUtcOffset()->getValue() : null,
                    'customFields' => $booking && $booking->getCustomFields() ?
