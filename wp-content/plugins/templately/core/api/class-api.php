...
class API {
	...
	public function get_dependencies() {
		$dependencies = DB::get_option( '_templately_dependencies', false );
		...
		if ( isset( $data['data'], $data['data']['dependencies'] ) ) {
			DB::update_option( '_templately_dependencies', $data['data']['dependencies'] );
		...
		if ( $type === 'pack' ) {
			$query = '{
                packs( slug: "' . $slug . '" ) {
                    ...
                    items {
                        slug, type, live_url, name, downloads, rating, id, dependencies{ id, icon, name, slug, is_pro, link, plugin_file, plugin_original_slug }, retina_ready, documentation_ready, features, favourite_count, thumbnail, price, pack{ name, slug }, author{ display_name, name, profile_photo, joined }, tags{ name }, category{ name, id } } }
                }
            }';
		}
	...
	protected function get_tags() {
		$tags = DB::get_option( '_templately_tags', false );
		if ( $tags !== false ) {
			return $tags;
		}
		...
		if ( isset( $data['data'], $data['data']['tags'] ) ) {
			DB::update_option( '_templately_tags', $data['data']['tags'] );
	...
	public function login_user() {
		...
		$global_signin = filter_var( $_POST['global_signin'], FILTER_VALIDATE_BOOLEAN);
		...
		if ( $withApi ) {
			...
			$query = \sprintf(
				'mutation{ connectWithApiKey( api_key : "%s", site_url : "%s", ip: "%s" ){ status, message, user { id, first_name, last_name, name, profile_photo, api_key, my_cloud { limit, usages, name, last_pushed, files{ data { id, name, thumbnail, medium_thumbnail, preview }, current_page, total_page }}, email, favourites { id, type }, joined, plan, plan_expire_at }}}',
				$api_key,
				\home_url( '/' ),
				self::get_ip()
			);
			...
			$query = \sprintf(
				'mutation{ connect( email : "%s", password : "%s", site_url : "%s", ip: "%s" ){ status, message, user { id, first_name, last_name, name, plan, plan_expire_at, profile_photo, api_key, email, favourites { id, type }, joined, my_cloud { limit, usages, name, last_pushed, files{ data { id, name, thumbnail, medium_thumbnail, preview }, current_page, total_page }}}}}',
				$email,
				$password,
				\home_url( '/' ),
				self::get_ip()
