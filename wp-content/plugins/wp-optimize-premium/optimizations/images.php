...
class WP_Optimization_images extends WP_Optimization {
	...
	protected $image_filename_regexp = '/^(.+)\-?(\d+x\d+|scaled|rotated)?(\@\dx)?(\.\w+)$/U';
					...
					if (!empty($posts_images_ids)) {
						...
						foreach ($unused_images[$blog_id] as &$image) {
							if (!array_key_exists('url', $image)) {
								...
								$image['url'] = utf8_encode($image_metadata['url']);
							}
						}
					}
	...
	private function get_multiple_image_ids_in_post_meta() {
		...
		$post_meta_names = apply_filters('wpo_get_multiple_image_ids_in_post_meta', $this->get_acf_field_names('gallery'));
		...
		if ($handle = opendir($base_upload_dir.'/'.$sub_directory)) {
			...
			while (false !== $file) {
				...
				if (false === $file || (($current_query_length + $static_query_length) >= $max_packet_size) || ($free_memory < $keep_free_memory && count($_image_files[$original_file_name]) > $min_images_per_check)) {
					...
					// if image not found in database save it as unused.
					foreach ($found_images as $original_file_name => $attachment_id) {
						if (!$attachment_id) {
							foreach ($_image_files[$original_file_name] as $file_name) {
								// check to avoid some filesystem errors.
								if (!is_file($base_upload_dir.'/'.$file_name)) continue;

								$unused_images[htmlentities($file_name)] = filesize($base_upload_dir.'/'.$file_name);
							}
						}
					}
					unset($_image_files);
	...
	public function get_image_attachment_id_bulk($filenames, $return_nonexistent = false) {
		...
		$query = "SELECT post_id, meta_value FROM {$wpdb->postmeta} pm WHERE pm.meta_key='_wp_attached_file' AND pm.meta_value IN (\"".join('","', $filenames)."\")";
		...
		$filenames_scaled_rotated = array();
		foreach ($filenames as $filename) {
			if (!array_key_exists($filename, $found_attachments)) {
				preg_match($this->image_filename_regexp, $filename, $match);
				if ('scaled' != $match[2] && 'rotated' != $match[2]) {
					$filenames_scaled_rotated[] = $match[1] . '-scaled' . $match[4];
					$filenames_scaled_rotated[] = $match[1] . '-rotated' . $match[4];
				}
			}
		}

		// trying to find in the database images with `-scaled`, `-rotated` suffix.
		if (!empty($filenames_scaled_rotated)) {
			$query = "SELECT post_id, meta_value FROM {$wpdb->postmeta} pm WHERE pm.meta_key='_wp_attached_file' AND pm.meta_value IN (\"".join('","', $filenames_scaled_rotated)."\")";
			$query_result = $wpdb->get_results($query, ARRAY_A);
		...
		if ($return_nonexistent) {
			...
			foreach ($filenames as $filename) {
				if (!array_key_exists($filename, $found_attachments)) $found_attachments[$filename] = false;
			}
		}
