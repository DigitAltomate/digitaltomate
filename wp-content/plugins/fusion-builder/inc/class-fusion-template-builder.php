...
class Fusion_Template_Builder {
	...
	public function register_post_types() {
		...
		$labels = [
			'name'                     => _x( 'Avada Builder Layouts', 'Layout general name', 'fusion-builder' ),
		...
		$labels = [
			'name'                     => _x( 'Avada Builder Sections', 'Section type general name', 'fusion-builder' ),
		...
		if ( $header_override && ! is_page_template( 'blank.php' ) ) {
			add_action(
				...
				function() use ( $header_override ) {
					...
					echo '<' . sanitize_key( $tag ) . ' class="fusion-tb-header">';
	...
	public function builder_check_archive_condition( $condition ) {
		...
		if ( '' === $sub_condition ) {
			...
			if ( ! is_admin() ) {
				...
				if ( 'archives' === $condition_type && taxonomy_exists( $archive_type ) && ! is_null( get_queried_object() ) ) {
					return $exclude ? ! ( get_queried_object()->taxonomy === $archive_type ) : get_queried_object()->taxonomy === $archive_type;
				}
			}
	...
	public function init_shortcodes() {
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			require_once FUSION_BUILDER_PLUGIN_DIR . 'shortcodes/components/woo-notices.php';
			...
			require_once FUSION_BUILDER_PLUGIN_DIR . 'shortcodes/components/woo-upsells.php';
			require_once FUSION_BUILDER_PLUGIN_DIR . 'shortcodes/components/woo-coming-soon.php';
		}
	}
	...
	public function load_component_templates() {
		...
		include FUSION_BUILDER_PLUGIN_DIR . '/front-end/templates/components/fusion-tb-woo-notices.php';
		...
		include FUSION_BUILDER_PLUGIN_DIR . '/front-end/templates/components/fusion-tb-woo-upsells.php';
	}
	...
	public function load_component_views() {
		...
		wp_enqueue_script( 'fusion_builder_tb_woo_notices', FUSION_BUILDER_PLUGIN_URL . 'front-end/views/components/view-woo-notices.js', [], FUSION_BUILDER_VERSION, true );
		...
		wp_enqueue_script( 'fusion_builder_tb_woo_upsells', FUSION_BUILDER_PLUGIN_URL . 'front-end/views/components/view-woo-upsells.js', [], FUSION_BUILDER_VERSION, true );
	}
	...
	public function builder_trigger( $admin_bar ) {
		...
		$admin_bar->add_node(
			[
				'id'    => 'fb-edit',
				'title' => esc_html__( 'Avada Live', 'fusion-builder' ),
				'href'  => $customize_url,
			]
		);
